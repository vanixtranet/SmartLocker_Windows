...
class _LockerSelectionScreenState extends State<LockerSelectionScreen> {
  ...
  listapiCall() async {
    widget.isReturn
        ? await adminBloc.getReturnedBayList(
            queryparams: {
              "actionName": "",
        ...
        : await adminBloc.getVacantListUser(
            queryparams: {
              "methodName": "",
              "transactionId": BlocProvider.of<UserModelBloc>(context)
                      .state
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  Text(
                    widget.isReturn
                        ? "Please select locker to take the returned device."
                        : "Please select locker to store the device.",
  ...
  checkLockerStatus() async {
    ...
    if (receivedData.contains("9007920101${binName}03")) {
      ...
      if (widget.isReturn) updateCollectedReturnItemsapiCall();
  ...
  updateCollectedReturnItemsapiCall() async {
    await lockerBloc.getUpdateCollectedReturnItems(
      queryparams: {
        "actionName": "ReturnItemCollected",
        "transactionId":
